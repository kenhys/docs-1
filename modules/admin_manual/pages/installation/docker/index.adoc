...
When running under docker, the admin user cannot control certain settings in the WebUI, instead they are now controlled by environment variables.
Changing these variables requires stopping and restarting the container with extra `docker -e ...` parameters, or with new entries in docker-compose's `.env` file.

==== Logging
The loglevel is fixed at 2, "Warnings, errors, and fatal issues".
